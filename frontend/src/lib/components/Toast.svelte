<script lang="ts">
	// Toast notification component for displaying global messages
	import { toastStore, type Toast } from '$lib/stores/toast';
	import { fade, fly } from 'svelte/transition';
	
	let toasts: Toast[] = [];
	
	toastStore.subscribe(store => {
		toasts = store.toasts;
	});
	
	function getToastStyles(type: Toast['type']) {
		switch (type) {
			case 'success':
				return {
					bg: 'bg-green-50',
					border: 'border-green-200',
					icon: 'text-green-400',
					text: 'text-green-800'
				};
			case 'error':
				return {
					bg: 'bg-red-50',
					border: 'border-red-200',
					icon: 'text-red-400',
					text: 'text-red-800'
				};
			case 'warning':
				return {
					bg: 'bg-yellow-50',
					border: 'border-yellow-200',
					icon: 'text-yellow-400',
					text: 'text-yellow-800'
				};
			case 'info':
				return {
					bg: 'bg-blue-50',
					border: 'border-blue-200',
					icon: 'text-blue-400',
					text: 'text-blue-800'
				};
		}
	}
	
	function getIcon(type: Toast['type']) {
		switch (type) {
			case 'success':
				return 'M10 18a8 8 0 100-16 8 8 0 000 16zm3.707-9.293a1 1 0 00-1.414-1.414L9 10.586 7.707 9.293a1 1 0 00-1.414 1.414l2 2a1 1 0 001.414 0l4-4z';
			case 'error':
				return 'M10 18a8 8 0 100-16 8 8 0 000 16zM8.707 7.293a1 1 0 00-1.414 1.414L8.586 10l-1.293 1.293a1 1 0 101.414 1.414L10 11.414l1.293 1.293a1 1 0 001.414-1.414L11.414 10l1.293-1.293a1 1 0 00-1.414-1.414L10 8.586 8.707 7.293z';
			case 'warning':
				return 'M8.257 3.099c.765-1.36 2.722-1.36 3.486 0l5.58 9.92c.75 1.334-.213 2.98-1.742 2.98H4.42c-1.53 0-2.493-1.646-1.743-2.98l5.58-9.92zM11 13a1 1 0 11-2 0 1 1 0 012 0zm-1-8a1 1 0 00-1 1v3a1 1 0 002 0V6a1 1 0 00-1-1z';
			case 'info':
				return 'M18 10a8 8 0 11-16 0 8 8 0 0116 0zm-7-4a1 1 0 11-2 0 1 1 0 012 0zM9 9a1 1 0 000 2v3a1 1 0 001 1h1a1 1 0 100-2v-3a1 1 0 00-1-1H9z';
		}
	}
</script>

<!-- Toast container -->
<div
	class="fixed top-4 right-4 z-50 flex flex-col gap-2 pointer-events-none"
	aria-live="polite"
	aria-atomic="true"
>
	{#each toasts as toast (toast.id)}
		{@const styles = getToastStyles(toast.type)}
		<div
			transition:fly={{ x: 300, duration: 300 }}
			class="pointer-events-auto max-w-sm w-full {styles.bg} border {styles.border} rounded-lg shadow-lg p-4"
			role="alert"
		>
			<div class="flex items-start gap-3">
				<svg
					class="h-5 w-5 flex-shrink-0 {styles.icon}"
					fill="currentColor"
					viewBox="0 0 20 20"
					aria-hidden="true"
				>
					<path fill-rule="evenodd" d={getIcon(toast.type)} clip-rule="evenodd" />
				</svg>
				<div class="flex-1 {styles.text} text-sm">
					{toast.message}
				</div>
				<button
					type="button"
					onclick={() => toastStore.remove(toast.id)}
					class="-mr-1 -mt-1 p-1 rounded-md {styles.text} hover:bg-black/5 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-offset-transparent focus:ring-gray-400"
					aria-label="Dismiss"
				>
					<svg class="h-4 w-4" fill="currentColor" viewBox="0 0 20 20" aria-hidden="true">
						<path
							fill-rule="evenodd"
							d="M4.293 4.293a1 1 0 011.414 0L10 8.586l4.293-4.293a1 1 0 111.414 1.414L11.414 10l4.293 4.293a1 1 0 01-1.414 1.414L10 11.414l-4.293 4.293a1 1 0 01-1.414-1.414L8.586 10 4.293 5.707a1 1 0 010-1.414z"
							clip-rule="evenodd"
						/>
					</svg>
				</button>
			</div>
		</div>
	{/each}
</div>
